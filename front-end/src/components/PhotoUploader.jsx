...
const PhotoUploader = ({ photolink, setPhotoLink, setPhotos, photos }) => {
  const uploadByLink = async (e) => {
    ...
    if (photolink) {
      try {
        ...
        alert("Error uploading link photos:", JSON.stringify(error));
      }
    } else {
  ...
  const uploadPhoto = async (e) => {
    ...
    try {
      ...
      setPhotos((prev) => [...prev, ...urlArray]);
    } catch (error) {
      alert("Error uploading photos:", JSON.stringify(error));
    }
  };
          ...
          <div key={photo} className="relative">
            ...
            <div className="absolute right-2 bottom-2 flex gap-2">
              <div
                ...
                className="cursor-pointer rounded-full border border-gray-400 bg-gray-200 p-0.5 opacity-65 hover:bg-red-300 hover:text-white"
              >
                <svg
